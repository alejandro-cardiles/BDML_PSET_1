...
table_categorical %>% as_latex() %>% write_lines(x = .,file = '05_visual/03_output/01_tabla_categoricas.tex')

##==: 5. Reimport tables
latex_table_continuous = read_lines('05_visual/03_output/01_tabla_continuas.tex')
latex_table_categoricas = read_lines('05_visual/03_output/01_tabla_categoricas.tex')

### Subset tables
latex_table_continuous = latex_table_continuous[3:16]
latex_table_categoricas = latex_table_categoricas[3:62]
...
latex_table_categoricas = append(c("\\centering \\footnotesize \\setlength{\\tabcolsep}{4pt} \\renewcommand{\\arraystretch}{0.8}",
                                  "\\begingroup",
                                  "\\fontsize{7.0pt}{8.5pt}\\selectfont"),
                                 latex_table_categoricas)

latex_table_categoricas = append(latex_table_categoricas,
                                 c('\\begin{minipage}{0.8\\textwidth}
                                   \\begin{tablenotes}[scriptsize,flushleft]
...
##==: 6. Reexport tables
latex_table_continuous %>% write_lines(x = .,file = '05_visual/03_output/01_tabla_continuas.tex')
latex_table_categoricas %>% write_lines(x = .,file = '05_visual/03_output/01_tabla_categoricas.tex')
